...
;; 4 PTS
(define (divides? a b)
  (cond [(and (equal? 0 a) (equal? 0 b)) #t]
      [(and (equal? 0 a) (not (equal? 0 b))) #f]
      [(equal? 0 (modulo b a)) #t]
      [else #f]))
...
(define (wj-solvable? cA cB g)
  (cond [(and  (or (<= g cA) (<= g cB)) (equal? 0 (modulo g (gcd cA cB)))) #t]
        [else #f]))
...
(define (help curA curB cA cB g)
  (cond [(equal? curB g) (list (list curA curB))]
        [(equal? curB cB) (cons (list curA curB) (help curA 0 cA cB g))]
        [(equal? curA 0) (cons (list curA curB) (help cA curB cA cB g))]
        [(> (+ curA curB) cB) (cons (list curA curB) (help (- curA (- cB curB)) cB cA cB g))]
        [(<= (+ curA curB) cB) (cons (list curA curB) (help 0 (+ curB curA) cA cB g))]))
