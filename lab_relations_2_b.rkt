...
(define (transitive-closure R)
  (union R (compose (transitive-help R (cardinality R)) (transitive-help R (cardinality R)))))

(define (transitive-help R n)
  (if (equal? 0 n)
      R
      (union (transitive-help R (- n 1)) (power R n))))
...
(define (equivalence? n R)
  (and (and (subset? (reflexive-closure n R) R) (transitive? R)) (symmetric? R)))
